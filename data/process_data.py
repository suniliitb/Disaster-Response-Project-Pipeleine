...
def load_data(messages_filepath, categories_filepath):
    ''' 
    function to read the messages and categories csv files and create an integrated dataset
    messages_filepath : path for messages csv file
    categories_filepath : path for categories csv file
    df: is the final integrated dataset returned by the function
    '''
...
def clean_data(df):
    ...
    '''
    Function to clean and organize the data by following below steps:
...
def save_data(df, database_filepath):
    '''
    Function to save the dataset as a sql lite database
    '''
